...
function useSeek() {
  ...
  const seek = async (url) => {
    let { access_token } = await fetch(`${process.env.REACT_APP_ENDPOINT}/token`, tokenOptions).then(res => res.json())
    await fetch(`${process.env.REACT_APP_ENDPOINT}${url}`, { headers: { "Authorization": `Bearer ${access_token}` } })
      .then(res => res.json())
